...
//               가장 작은 프로세스를 찾는다.
		...
		for (int i = 0; i < readyQueue.size(); i++) {
			Process processAtNow = readyQueue.get(i);
			int timeProcessAtNow = processAtNow.getServiceTime();
...
//               가장 작은 프로세스를 찾는다.
		...
		for (int i = 0; i < readyQueue.size(); i++) {
			...
			// 실행시간
//            int timeExecution = processAtNow.getServiceTime() - processAtNow.getRemainingTime();
			int timeExecution = super.currentTime;
			// 기다린 시간
			int timeSpentWaitAtNow = processAtNow.getWaitingTime(timeExecution);

			// 서비스시간
			int timeService = processAtNow.getServiceTime();

			double HRRN = (double) (timeSpentWaitAtNow + timeService) / timeService;
...
//          찾은 프로세스를 currentProcess에 설정한다.
		...
		for (int i = 0; i < readyQueue.size(); i++) {
			Process processAtNow = readyQueue.get(i);
			int timeRemaining = processAtNow.getRemainingTime();
...
//          새로 도착한 프로세스가 있는 경우 true를 반환 (리턴문을 한 문장으로 코딩해 보라)
	@Override
	public boolean isSchedulable() {
		return super.isSchedulable() || super.isNewProcessArrived;
	}
}
	...
	public void run(Scheduler scheduler) { // 각 스케줄링 알고리즘을 테스트 함
		...
		while (scheduler.hasMoreProcessesToExecute()) { // 아직 더 실행해야 할 프로세스가 있는지 체크
			...
			try { // 우리 스케줄러에서 사용할 시간단위는 100ms: 빠르게 실행하려면 이 값을 10 또는 1로 줄여도 됨
				...
				Thread.sleep(10); // 100 millisecond 동안 정지했다가 리턴함 // 8장에서 시간 수정 100 -> 10
